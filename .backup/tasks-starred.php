...
<div class="table-container">
  ...
  $info_sql = "SELECT * FROM `mytasks` WHERE id ='".$id."'"." AND starred = '1' AND currentStatus!='Completed'";
  $info_result = mysqli_query($conn, $info_sql);
   // $row = mysqli_fetch_assoc($info_result); // --> remember not to use mysqli_fetch_assoc twice

    // Display the information in the HTML table
    if (mysqli_num_rows($info_result) > 0) {
        while ($row = mysqli_fetch_assoc($info_result)) {
               echo "<tr data-task-id='".$row["taskID"]."'>";
               echo "<td><input type='checkbox' name='Completed' value='Completed'onchange='updateTaskStatus(".$row["taskID"].",this.checked)'></td>";
               echo "<td>
               <h3>".$row["taskName"]."</h3>
