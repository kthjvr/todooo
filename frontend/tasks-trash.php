        ...
        <div class="task-list">
          ...
          <ul id="task-list">
            ...
            <?php
                ...
                // $info_sql = "SELECT * FROM mytasks JOIN categories WHERE categories.categoryID=mytasks.categoryID AND categories.id=$id 
                // AND mytasks.id=$id AND mytasks.trash='1' AND mytasks.currentStatus='Discarded'"; //retrieve info from db

                $info_sql = "SELECT mytasks.*, categories.category
                FROM mytasks
                JOIN categories ON categories.categoryID = mytasks.categoryID
                WHERE (mytasks.id = $id AND mytasks.trash = '1' AND mytasks.currentStatus = 'Discarded')
                ...
                JOIN categories ON categories.categoryID = mytasks.categoryID
                WHERE (assignments.assignee_id = $id AND mytasks.trash = '1' AND mytasks.currentStatus = 'Discarded')";
...
<script>
  ...
  if (taskList.children().length == 0) { // Check if the task list is empty
    ...
    $.ajax({
      url: "../backend/trash_get_task_details.php",
      type: "POST",
