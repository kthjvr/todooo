      ...
      <table id="myTable">
        ...
        <tbody id="table-body">
          <?php 
            ...
            $info_sql = "SELECT * FROM mytasks JOIN categories WHERE categories.categoryID=mytasks.categoryID AND categories.id=mytasks.id AND mytasks.trash='0' AND mytasks.currentStatus!='Completed' AND mytasks.endDate='".$current_date."'"; //retrieve info from db
            // $info_sql = "SELECT * FROM MyTasks WHERE id = '$id' AND currentStatus!='Completed' AND trash='0' AND endDate='".$current_date."'"; //retrieve info from db
            ...
            <?php
              ...
              if (mysqli_num_rows($info_result) > 0) { // Display the information in the HTML table
                  while ($row = mysqli_fetch_assoc($info_result)) {
                      ...
                      echo "<option value='".$row["categoryID"]."' name='".$row["category"]."' id='".$row["category"]."'>".$row["category"]."</option>";
                  }
              } else {
